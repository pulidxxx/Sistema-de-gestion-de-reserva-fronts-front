...
class ContenedorReservasMaterial extends Contenedor {
    render(): JSX.Element {
        ...
        const obtenerMateriales = async () => {
            try {
                const response = await fetch(
                    `${API_BASE_URL}/reservas-material/byEmail/${email}`
                );
                if (!response.ok)
        ...
        const handleDelete = async (material) => {
            ...
            try {
                const response = await fetch(
                    `${API_BASE_URL}/materiales/eliminar/${material.id}`,
                    {
                        method: "DELETE",
                        headers: {
        ...
        const handleCalificar = async (material, calificacion, comentario) => {
            try {
                const response = await fetch(
                    `${API_BASE_URL}/reservas-material/calificar/${material.id}`,
                    {
                        method: "PATCH",
                        headers: {
                            "Content-Type": "application/json",
                ...
                if (!response.ok)
                    throw new Error("Error al calificar el material");

                // Actualizar el estado del material
                setMateriales((prev) =>
                    prev.map((m) =>
                        m.id === material.id
                            ? { ...m, calificacion, comentario }
                            : m
                    )
                );
            } catch (error) {
            ...
            <>
                ...
                <Modal show={showCalificacion} onHide={handleCloseCalificacion}>
                    ...
                    <Modal.Body>
                        ...
                        <p>
                            Calificación:{" "}
                            {material?.calificacion || "No calificado"}
                        </p>
                        <p>
                            Comentario:{" "}
                            {material?.comentario ||
